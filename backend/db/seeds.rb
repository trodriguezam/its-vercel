...
Question.create(task_id: task4.id, question_text: "Calcula el centro de gravedad de un sistema de dos masas de {valor1}kg a 2m y {valor2}kg a 4m", hint:
...
Question.create(task_id: task5.id, question_text: "Determina el centro de gravedad de un sistema de 3 masas en un plano cartesiano, con m1=2kg en ({distancia1}, {distancia2}), m2=3kg en ({distancia3}, {distancia4}), y m3=1kg en ({distancia5}, {distancia6}). La respuesta debe estar en formato (x,y)", hint:
...
Question.create(task_id: task4.id, question_text: "Calcula la fuerza de fricción que actúa sobre un objeto de {valor1}kg en una superficie con un coeficiente de fricción de 0.3. Asume g = 10 m/s². Se asume una respuesta en [N]", hint:
...
Question.create(task_id: task5.id, question_text: "Determina la fuerza neta que actúa sobre un objeto m1 si se le aplica una fuerza de {valor1}N hacia la derecha y una de {valor2}N hacia la izquierda. Se asume una respuesta en [N]", hint:
...
Question.create(task_id: task4.id, question_text: "Un bloque de {valor1}kg está en reposo suspendido por una cuerda vertical. Calcula la tensión en la cuerda. Asume g = 10 m/s². Se asume una respuesta en [N]", hint:
...
Question.create(task_id: task5.id, question_text: "Un objeto de {valor1}kg está sobre una pendiente inclinada 30° respecto a la horizontal. Calcula la magnitud de la fuerza normal que actúa sobre el objeto. Asume g = 10 m/s². Se asume una respuesta en [N]", hint:
...
Question.create(task_id: task4.id, question_text: "Si hay una fuerza a {distancia1} metros del pivote, cuánto tendría que valer esta para que la barra de 4 metros, con un peso de 100 N, esté en equilibrio. Se asume una respuesta en [N]", hint:
...
Question.create(task_id: task5.id, question_text: "Determina el momento necesario para equilibrar un objeto que tiene una fuerza de {valor1}N aplicada a 2 metros del eje de rotación. Se asume una respuesta en [N·m]", hint:
...
Question.create(task_id: task4.id, question_text: "Calcula la fuerza necesaria para que un bloque de {valor1}kg entre en movimiento en una superficie con un coeficiente de roce de 0.2. Se asume una respuesta en [N]", hint:
...
Question.create(task_id: task5.id, question_text: "Cuánto debiese valer la fuerza que actúa sobre m1 para que el sistema esté en equilibrio, asumiendo que m2 vale {valor1}kg. Asume g = 10 m/s². Se asume una respuesta en [N]", hint:
